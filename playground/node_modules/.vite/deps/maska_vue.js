...
var R = {
	...
	const h = i.split(".");
	if (h[1] != null && h[1].length >= 1) a = w(h[1].length <= r ? h[1].length : r, r, e);
...
var O = class {
	constructor(t = {}) {
		...
		const e = { ...t };
		if (e.tokens != null) {
			e.tokens = e.tokensReplace ? { ...e.tokens } : {
				...R,
				...e.tokens
			};
			for (const s of Object.values(e.tokens)) typeof s.pattern == "string" && (s.pattern = I() ? new RegExp(s.pattern, "u") : new RegExp(s.pattern));
		} else e.tokens = R;
	...
	process(t, e, s = !0) {
		...
		for (; k();) {
			...
			if (!u.includes(o) && m != null ? (y.match(m.pattern) != null ? (p[h](y), m.repeated ? (d === -1 ? d = o : o === l && o !== d && (o = d - i), l === d && (o -= i)) : m.multiple && (M = !0, o -= i), o += i) : m.multiple ? M && (o += i, c -= i, M = !1) : y === b ? b = void 0 : m.optional && (o += i, c -= i), c += i) : (s && !this.isEager() && p[h](v), y === v && !this.isEager() ? c += i : b = v, this.isEager() || (o += i)), this.isEager()) for (; g(o) && (f[a.charAt(o)] == null || u.includes(o));) {
				if (s) {
	...
	init(t) {
		...
		for (const s of t) {
			if (!this.items.has(s)) {
				...
				s.addEventListener("input", this.onInput, {
					capture: !0,
					signal: a
				});
			}
