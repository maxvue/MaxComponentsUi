import { G as createBaseVNode, Ht as openBlock, J as createElementBlock, Jt as renderList, K as createBlock, N as Fragment, Ot as mergeProps, Qt as resolveDynamicComponent, q as createCommentVNode } from "./runtime-dom.esm-bundler-mXmJYp7I.js";
import "./vue.js";
import { It as f, Mt as v, U as C, dt as V, lt as Tt, ot as Rt, sn as s, zt as C$1 } from "./dist-DAdOYTgg.js";
import { t as BaseStyle } from "./style-BrUChsnA.js";
import { t as script$2 } from "./basecomponent-BglYeR_S.js";
import { r as getVNodeProp } from "./utils-DESbX76T.js";
//#endregion
//#region node_modules/primevue/splitter/style/index.mjs
...
//#endregion
//#region node_modules/primevue/splitter/index.mjs
var script$1 = {
	name: "BaseSplitter",
	"extends": script$2,
	props: {
...
var script = {
	...
	watch: { panelSizeConfig: function panelSizeConfig() {
		this.initializePanels();
	} },
	...
	methods: {
		...
		getPanelSize: function getPanelSize(panel) {
			var size = panel.props && s(panel.props.size) ? panel.props.size : null;
		...
		onResize: function onResize(event, step, isKeyDown) {
			...
			if (isKeyDown) if (this.horizontal) {
				...
				if (this.horizontal) if (V(this.$el)) newPos = (this.startPos - event.pageX) * 100 / this.size;
				else newPos = (event.pageX - this.startPos) * 100 / this.size;
				else newPos = (event.pageY - this.startPos) * 100 / this.size;
		...
		onGutterKeyDown: function onGutterKeyDown(event, index) {
			switch (event.code) {
				case "ArrowLeft":
					if (this.layout === "horizontal") this.setTimer(event, index, this.step * -1);
					event.preventDefault();
					break;
				case "ArrowRight":
					if (this.layout === "horizontal") this.setTimer(event, index, this.step);
					event.preventDefault();
					break;
				case "ArrowDown":
					if (this.layout === "vertical") this.setTimer(event, index, this.step * -1);
					event.preventDefault();
					break;
				case "ArrowUp":
					if (this.layout === "vertical") this.setTimer(event, index, this.step);
					event.preventDefault();
					break;
			}
		},
		...
		validateResize: function validateResize(newPrevPanelSize, newNextPanelSize) {
			...
			if (this.prevPanelMinSize > newPrevPanelSize) return false;
			if (this.nextPanelMinSize > newNextPanelSize) return false;
			return true;
		},
		...
		saveState: function saveState() {
			if (C$1(this.panelSizes)) this.getStorage().setItem(this.stateKey, JSON.stringify(this.panelSizes));
		},
		restoreState: function restoreState() {
			...
			if (stateString) {
				...
				_toConsumableArray(this.$el.children).filter(function(child) {
					return child.getAttribute("data-pc-name") === "splitterpanel";
	...
	computed: {
		panels: function panels() {
			...
			this.$slots["default"]().forEach(function(child) {
				if (_this6.isSplitterPanel(child)) panels.push(child);
				else if (child.children instanceof Array) child.children.forEach(function(nestedChild) {
					if (_this6.isSplitterPanel(nestedChild)) panels.push(nestedChild);
				});
			});
			return panels;
		},
		...
		prevPanelMinSize: function prevPanelMinSize() {
			...
			if (this.panels[this.prevPanelIndex].props && prevPanelMinSize) return prevPanelMinSize;
			return 0;
		},
		nextPanelMinSize: function nextPanelMinSize() {
			var nextPanelMinSize = getVNodeProp(this.panels[this.prevPanelIndex + 1], "minSize");
			if (this.panels[this.prevPanelIndex + 1].props && nextPanelMinSize) return nextPanelMinSize;
			return 0;
		},
		dataP: function dataP() {
			var _this$$parentInstance2;
			return f(_defineProperty(_defineProperty({}, this.layout, this.layout), "nested", ((_this$$parentInstance2 = this.$parentInstance) === null || _this$$parentInstance2 === void 0 ? void 0 : _this$$parentInstance2.nestedState) != null));
		}
	}
};
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
	return openBlock(), createElementBlock("div", mergeProps({
		...
		return openBlock(), createElementBlock(Fragment, { key: i }, [(openBlock(), createBlock(resolveDynamicComponent(panel), { tabindex: "-1" })), i !== $options.panels.length - 1 ? (openBlock(), createElementBlock("div", mergeProps({
			...
			"data-p": $options.dataP
		}, { ref_for: true }, _ctx.ptm("gutter")), [createBaseVNode("div", mergeProps({
