import { E as withModifiers, G as createBaseVNode, Gn as reactive, Ht as openBlock, J as createElementBlock, Jt as renderList, K as createBlock, N as Fragment, Ot as mergeProps, Qt as resolveDynamicComponent, Sn as withCtx, Tr as normalizeClass, Xt as resolveComponent, Yt as renderSlot, Zt as resolveDirective, et as createTextVNode, jr as toDisplayString, q as createCommentVNode, t as Transition, tt as createVNode, wn as withDirectives } from "./runtime-dom.esm-bundler-mXmJYp7I.js";
import "./vue.js";
import { It as f, Mt as v, Pt as z, U as C, Xt as c, an as p, it as Q, mt as Y } from "./dist-DAdOYTgg.js";
import { t as BaseStyle } from "./style-BrUChsnA.js";
import { t as script$3 } from "./basecomponent-BglYeR_S.js";
import { t as script$4 } from "./chevronright-BQc2v3hB.js";
import { t as script$5 } from "./chevrondown-DwUJ3Cx9.js";
import { t as Ripple } from "./ripple-ChdnQOSB.js";
import { t as script$6 } from "./spinner-ChffWSa1.js";
import { t as script$7 } from "./inputtext-CyYxa08i.js";
import { t as script$8 } from "./check-C5YMRsEh.js";
import { t as script$9 } from "./minus-LxZxuBhb.js";
import { t as script$10 } from "./checkbox-BMZ639Qw.js";
import { t as script$11 } from "./search-BM2P3QWn.js";
import { t as script$12 } from "./iconfield-BeduG4tw.js";
import { t as script$13 } from "./inputicon-CPQPCqpa.js";
//#endregion
...
var TreeStyle = BaseStyle.extend({
	...
	classes: {
		...
		node: function node(_ref2) {
			return ["p-tree-node", { "p-tree-node-leaf": _ref2.instance.leaf }];
		},
...
function useTreeDragDropService() {
	return {
		...
		onDragStart: function onDragStart(handler) {
			dragStartHandlers.add(handler);
			return function() {
				return dragStartHandlers["delete"](handler);
			};
		},
		onDragStop: function onDragStop(handler) {
...
var script$1 = {
	...
	methods: {
		...
		onClick: function onClick(event) {
			if (this.toggleClicked || Q(event.target, "[data-pc-section=\"nodetogglebutton\"]") || Q(event.target.parentElement, "[data-pc-section=\"nodetogglebutton\"]")) {
				this.toggleClicked = false;
				return;
			}
			if (this.isCheckboxSelectionMode()) {
				if (this.node.selectable != false) this.toggleCheckbox();
			} else this.$emit("node-click", {
				originalEvent: event,
		...
		onArrowDown: function onArrowDown(event) {
			...
			if (listElement) this.focusRowChange(nodeElement, listElement.children[0]);
			else if (nodeElement.nextElementSibling) this.focusRowChange(nodeElement, nodeElement.nextElementSibling);
			else {
				var nextSiblingAncestor = this.findNextSiblingOfAncestor(nodeElement);
				if (nextSiblingAncestor) this.focusRowChange(nodeElement, nextSiblingAncestor);
			}
			event.preventDefault();
		},
		onArrowUp: function onArrowUp(event) {
			var nodeElement = event.target;
			if (nodeElement.previousElementSibling) this.focusRowChange(nodeElement, nodeElement.previousElementSibling, this.findLastVisibleDescendant(nodeElement.previousElementSibling));
			else {
				var parentNodeElement = this.getParentNodeElement(nodeElement);
				if (parentNodeElement) this.focusRowChange(nodeElement, parentNodeElement);
			}
			event.preventDefault();
		},
		...
		onArrowLeft: function onArrowLeft(event) {
			var togglerElement = z(event.currentTarget, "[data-pc-section=\"nodetogglebutton\"]");
			if (this.level === 0 && !this.expanded) return false;
			if (this.expanded && !this.leaf) {
				togglerElement.click();
				return false;
			}
			var target = this.findBeforeClickableNode(event.currentTarget);
			if (target) this.focusRowChange(event.currentTarget, target);
		},
		...
		removeNodeFromTree: function removeNodeFromTree(nodes, nodeToRemove) {
			...
			return nodes.reduce(function(acc, node) {
				if (node.key === nodeToRemove.key) return acc;
				if (node.children && node.children.length > 0) {
					var updatedChildren = _this2.removeNodeFromTree(node.children, nodeToRemove);
					acc.push(_objectSpread$2(_objectSpread$2({}, node), {}, { children: updatedChildren }));
				} else acc.push(node);
				return acc;
			}, []);
		},
		insertNodeInSiblings: function insertNodeInSiblings(nodes, targetKey, nodeToInsert, offset) {
			...
			if (targetIndex !== -1) return nodes.toSpliced(targetIndex + offset, 0, nodeToInsert);
			return nodes.map(function(node) {
				if (node.children && node.children.length > 0) return _objectSpread$2(_objectSpread$2({}, node), {}, { children: _this3.insertNodeInSiblings(node.children, targetKey, nodeToInsert, offset) });
				return node;
			});
		},
		addNodeAsChild: function addNodeAsChild(nodes, parentKey, nodeToInsert) {
			var _this4 = this;
			return nodes.map(function(node) {
				if (node.key === parentKey) return _objectSpread$2(_objectSpread$2({}, node), {}, { children: [].concat(_toConsumableArray$1(node.children || []), [nodeToInsert]) });
				if (node.children && node.children.length > 0) return _objectSpread$2(_objectSpread$2({}, node), {}, { children: _this4.addNodeAsChild(node.children, parentKey, nodeToInsert) });
				return node;
			});
		},
		insertNodeOnDrop: function insertNodeOnDrop() {
			...
			if (!this.node || dragNodeIndex == null || !dragNode || !dragNodeSubNodes) return null;
			var position = this.dropPosition;
			var updatedNodes = this.removeNodeFromTree(this.rootNodes, dragNode);
			if (position < 0) updatedNodes = this.insertNodeInSiblings(updatedNodes, this.node.key, dragNode, 0);
			else if (position > 0) updatedNodes = this.insertNodeInSiblings(updatedNodes, this.node.key, dragNode, 1);
			else updatedNodes = this.addNodeAsChild(updatedNodes, this.node.key, dragNode);
		...
		onNodeDrop: function onNodeDrop(event) {
			...
			if (this.isDroppable) {
				...
				var position = this.dropPosition;
				if (position !== 0 || position === 0 && this.isNodeDroppable) if (this.validateDrop) this.$emit("node-drop", {
					originalEvent: event,
		...
		onNodeDragOver: function onNodeDragOver(event) {
			if (this.isDroppable) {
				...
				this.isNextDropPointHovered = false;
				this.isNodeDropHovered = false;
				if (y < rect.height * .25) this.isPrevDropPointHovered = true;
				else if (y > rect.height * .75) this.isNextDropPointHovered = true;
				else if (this.isNodeDroppable) this.isNodeDropHovered = true;
		...
		onNodeDragLeave: function onNodeDragLeave() {
			this.$emit("node-dragleave", { node: this.node });
			this.isPrevDropPointHovered = false;
			this.isNextDropPointHovered = false;
			this.isNodeDropHovered = false;
		},
		...
		setAllNodesTabIndexes: function setAllNodesTabIndexes() {
			var nodes = Y(this.$refs.currentNode.closest("[data-pc-section=\"rootchildren\"]"), "[role=\"treeitem\"]");
		...
		setTabIndexForSelectionMode: function setTabIndexForSelectionMode(event, nodeTouched) {
			if (this.selectionMode !== null) {
				var elements = _toConsumableArray$1(Y(this.$refs.currentNode.parentElement, "[role=\"treeitem\"]"));
		...
		findBeforeClickableNode: function findBeforeClickableNode(node) {
			...
			if (parentListElement) {
				...
				if (prevNodeButton && prevNodeButton.style.visibility !== "hidden") return parentListElement;
		...
		propagateUp: function propagateUp(event) {
			...
			if (check && checkedChildCount === this.node.children.length) _selectionKeys[this.node.key] = {
				checked: true,
				partialChecked: false
			};
			else {
				if (!check) delete _selectionKeys[this.node.key];
		...
		findNextSiblingOfAncestor: function findNextSiblingOfAncestor(nodeElement) {
			var parentNodeElement = this.getParentNodeElement(nodeElement);
			if (parentNodeElement) if (parentNodeElement.nextElementSibling) return parentNodeElement.nextElementSibling;
		...
		findLastVisibleDescendant: function findLastVisibleDescendant(nodeElement) {
			...
			} else return nodeElement;
		},
		getParentNodeElement: function getParentNodeElement(nodeElement) {
			var parentNodeElement = nodeElement.parentElement.parentElement;
			return Q(parentNodeElement, "role") === "treeitem" ? parentNodeElement : null;
		},
...
function render$1(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createElementBlock("li", mergeProps({
		...
		$options.isPrevDropPointActive ? (openBlock(), createElementBlock("div", {
			key: 0,
		...
		createBaseVNode("div", mergeProps({
			...
			withDirectives((openBlock(), createElementBlock("button", mergeProps({
			...
			}, $options.getPTOptions("nodeToggleButton")), [$props.node.loading && $props.loadingMode === "icon" ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [$props.templates["nodetoggleicon"] || $props.templates["nodetogglericon"] ? (openBlock(), createBlock(resolveDynamicComponent($props.templates["nodetoggleicon"] || $props.templates["nodetogglericon"]), {
				key: 0,
				node: $props.node,
			...
			}, $options.getPTOptions("nodeToggleIcon")), null, 16, ["class"]))], 64)) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [$props.templates["nodetoggleicon"] || $props.templates["togglericon"] ? (openBlock(), createBlock(resolveDynamicComponent($props.templates["nodetoggleicon"] || $props.templates["togglericon"]), {
				key: 0,
				node: $props.node,
				expanded: $options.expanded,
				"class": normalizeClass(_ctx.cx("nodeToggleIcon"))
				...
				"expanded",
				"class"
			])) : $options.expanded ? (openBlock(), createBlock(resolveDynamicComponent($props.node.expandedIcon ? "span" : "ChevronDownIcon"), mergeProps({
				key: 1,
				"class": _ctx.cx("nodeToggleIcon")
				...
				key: 2,
				"class": _ctx.cx("nodeToggleIcon")
			}, $options.getPTOptions("nodeToggleIcon")), null, 16, ["class"]))], 64))], 16, _hoisted_3$1)), [[_directive_ripple]]),
			$options.checkboxMode ? (openBlock(), createBlock(_component_Checkbox, {
				key: 0,
			...
			$props.templates["nodeicon"] ? (openBlock(), createBlock(resolveDynamicComponent($props.templates["nodeicon"]), mergeProps({
				key: 1,
				node: $props.node,
			...
			}, $options.getPTOptions("nodeIcon")), null, 16)),
			createBaseVNode("span", mergeProps({ "class": _ctx.cx("nodeLabel") }, $options.getPTOptions("nodeLabel"), { onKeydown: _cache[1] || (_cache[1] = withModifiers(function() {}, ["stop"])) }), [$props.templates[$props.node.type] || $props.templates["default"] ? (openBlock(), createBlock(resolveDynamicComponent($props.templates[$props.node.type] || $props.templates["default"]), {
				key: 0,
				node: $props.node,
				expanded: $options.expanded,
				...
				"expanded",
				"selected"
			])) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [createTextVNode(toDisplayString($options.label($props.node)), 1)], 64))], 16)
		], 16, _hoisted_2$1),
		$options.isNextDropPointActive ? (openBlock(), createElementBlock("div", {
			key: 1,
			"class": normalizeClass(_ctx.cx("dropPoint")),
			"aria-hidden": "true"
		}, null, 2)) : createCommentVNode("", true),
...
var script = {
	...
	watch: {
		...
		droppableNodes: function droppableNodes(newValue) {
			if (newValue) this.initDragDropService();
			else this.cleanupDragDropService();
		}
	},
	mounted: function mounted() {
		if (this.droppableNodes) this.initDragDropService();
	},
	...
	methods: {
		...
		isFilterMatched: function isFilterMatched(node, _ref) {
			...
			try {
				for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
					...
					if (String(p(node, field)).toLocaleLowerCase(this.filterLocale).indexOf(filterText) > -1) matched = true;
				}
			} catch (err) {
				_iterator2.e(err);
			} finally {
		...
		isDescendantOf: function isDescendantOf(node, target) {
			...
			try {
				for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
					...
					if (child === target || this.isDescendantOf(child, target)) return true;
				}
			} catch (err) {
				_iterator3.e(err);
			} finally {
		...
		allowDrop: function allowDrop(dragNode, dropNode, dragNodeScope) {
			...
			if (!this.isValidDragScope(dragNodeScope)) return false;
			if (dropNode && (dragNode === dropNode || this.isDescendantOf(dragNode, dropNode))) return false;
			return true;
		},
		...
		hasCommonScope: function hasCommonScope(dragScope, dropScope) {
			if (dragScope === null && dropScope === null) return true;
			else if (dragScope === null || dropScope === null) return false;
			if (typeof dropScope === "string") {
				if (typeof dragScope === "string") return dragScope === dropScope;
				else if (Array.isArray(dragScope)) return dragScope.indexOf(dropScope) !== -1;
			} else if (Array.isArray(dropScope)) {
				if (typeof dragScope === "string") return dropScope.indexOf(dragScope) !== -1;
				else if (Array.isArray(dragScope)) {
					var _iterator4 = _createForOfIteratorHelper(dragScope), _step4;
					try {
						for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
							var ds = _step4.value;
							if (dropScope.indexOf(ds) !== -1) return true;
						}
					} catch (err) {
						_iterator4.e(err);
					} finally {
		...
		isValidDragScope: function isValidDragScope(dragScope) {
			if (this.droppableScope === null) return true;
		...
		onDragOver: function onDragOver(event) {
			if (this.droppableNodes && this.allowDrop(this.dragNode, null, this.dragNodeScope)) event.dataTransfer.dropEffect = "copy";
			else event.dataTransfer.dropEffect = "none";
			event.preventDefault();
		},
		...
		onDragLeave: function onDragLeave(event) {
			if (this.droppableNodes) {
				var rect = event.currentTarget.getBoundingClientRect();
				if (event.x >= parseInt(rect.right) || event.x <= parseInt(rect.left) || event.y >= parseInt(rect.bottom) || event.y <= parseInt(rect.top)) this.dragHover = false;
	...
	computed: {
		filteredValue: function filteredValue() {
			var filteredNodes = [];
			var searchFields = c(this.filterBy) ? [this.filterBy] : this.filterBy.split(",");
			...
			try {
				for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
					...
					if (strict && (this.findFilteredNodes(_node, paramsWithoutNode) || this.isFilterMatched(_node, paramsWithoutNode)) || !strict && (this.isFilterMatched(_node, paramsWithoutNode) || this.findFilteredNodes(_node, paramsWithoutNode))) filteredNodes.push(_node);
				}
			} catch (err) {
				_iterator5.e(err);
			} finally {
		...
		wrapperDataP: function wrapperDataP() {
			return f({ scrollable: this.scrollHeight === "flex" });
		}
	},
	components: {
		TreeNode: script$1,
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return openBlock(), createElementBlock("div", mergeProps({
		...
		createVNode(Transition, { name: "p-overlay-mask" }, {
			"default": withCtx(function() {
				return [_ctx.loading && _ctx.loadingMode === "mask" ? (openBlock(), createElementBlock("div", mergeProps({
					key: 0,
					"class": _ctx.cx("mask")
				}, _ctx.ptm("mask")), [renderSlot(_ctx.$slots, "loadingicon", { "class": normalizeClass(_ctx.cx("loadingIcon")) }, function() {
		...
		_ctx.filter ? (openBlock(), createBlock(_component_IconField, {
			...
			"default": withCtx(function() {
				return [createVNode(_component_InputText, {
					...
					"default": withCtx(function() {
						return [renderSlot(_ctx.$slots, _ctx.$slots.filtericon ? "filtericon" : "searchicon", { "class": normalizeClass(_ctx.cx("filterIcon")) }, function() {
							return [createVNode(_component_SearchIcon, mergeProps({ "class": _ctx.cx("filterIcon") }, _ctx.ptm("filterIcon")), null, 16, ["class"])];
						})];
					}),
					_: 3
				}, 8, ["unstyled", "pt"])];
		...
		createBaseVNode("div", mergeProps({
			...
			style: { maxHeight: _ctx.scrollHeight },
...
//# sourceMappingURL=tree-Ce1hyrxx.js.map