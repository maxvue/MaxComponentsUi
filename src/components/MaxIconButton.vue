<template>
    <div class="icon-div ico-btn" ref="icon_ref" :icon="attrs.icon ?? attrs.i" v-bind="attrs">
        <MaxIcon v-bind="attrs" v-tooltip="null" pointer @click.stop="execute" v-if="attrs.blank || attrs.route" />
        <MaxIcon v-bind="attrs" v-tooltip="null" pointer v-else />
...
<script setup lang="ts">
    ...
    const execute = () => {
        ...
        if (attrs.route) {
            const data: any = { name: attrs.route };
            if (attrs.data ?? attrs.params) data['query'] = attrs.data ?? attrs.params;

            if (attrs.route === 'project' && !data.query?.id) {
                console.log('ROTA NÃO EXECUTADA');
                return;
            }
            router.push(data);
        }
