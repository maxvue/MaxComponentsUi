<template>
    <div class="icon-div ico-btn" ref="icon_ref" :icon="attrs.icon ?? attrs.i" v-bind="attrs" :style="{width: size, height: size}">
        <MaxIcon v-bind="attrs" v-tooltip="null" pointer @click.stop="execute" v-if="attrs.blank || attrs.route" full />
        <MaxIcon v-bind="attrs" v-tooltip="null" pointer v-else full />
...
<script setup lang="ts">
    ...
    const size = computed(() => 24 * Number(attrs.size ?? 1) + 'px');
</script>
