<template>
    <div class="max-input-html" v-bind="props">
        <div class="max-input-html__editor-wrap" :class="{ 'max-input-html__editor-wrap--disabled': props.disabled }">
            <MaxInputHtmlToolbar :editor="editor ?? null" :hide-tools="props.hideTools" :tools="props.tools" />
            <EditorContent class="max-input-html__content" :style="{ minHeight: props.minHeight, maxHeight: props.maxHeight }" :editor="editor" />
        </div>
...
<script setup lang="ts">
    ...
    import MaxInputHtmlToolbar, { type HtmlToolbarTool } from './MaxInputHtmlToolbar.vue';
    ...
    const modalId = 'max-input-html-lightbox-' + useId();
    ...
    const insertPdfFile = (file: File) => {
        ...
        if (uploadFn) uploadFn(file).then((url) => {
            if (url && isSafeUrl(url)) {
                ...
                editor.value?.chain().focus().insertContent(`<a href="${url}" target="_blank" rel="noopener noreferrer">${label}</a> `).run();
            }
        }).catch((err) => {
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                class: 'max-input-html__prosemirror',
    ...
    .max-input-html__content {
        ...
        :deep() {
            .max-input-html__prosemirror {
                ...
                // Compatibilidade com classes legadas do Quill e estilos de alinhamento
                .ql-align-justify, [style*='text-align: justify'], [style*='text-align:justify'] {
                    text-align: justify;
                }

                .ql-align-center, [style*='text-align: center'], [style*='text-align:center'] {
                    text-align: center;
                }

                .ql-align-right, [style*='text-align: right'], [style*='text-align:right'] {
                    text-align: right;
                }

                .ql-align-left, [style*='text-align: left'], [style*='text-align:left'] {
                    text-align: left;
                }

                // Suporte para spans sem quebra de linha (ex: CPFs, telefones, datas)
                [nowrap], [style*='white-space: nowrap'], [style*='white-space:nowrap'], .nowrap {
                    white-space: nowrap;
                }
                ...
                // Inline
                strong, b, [style*='font-weight: bold'], [style*='font-weight:bold'] {
                    font-weight: 700;
                    color: inherit;
                }
                ...
                code {
                    ...
                    color: var(--max-primary-700, #004860);
