...
describe('MaxInputFileUpload', () => {
    ...
    describe('Invalidar Geração no Unmount e Impedir Callbacks Tardios (F17 / E07-03)', () => {
        it('limpa handlers de evento antes de abortar XHR ativo no unmount', () => {
            let createdXhr: any = null;
            const abortSpy = vi.fn();
            vi.spyOn(XMLHttpRequest.prototype, 'open').mockImplementation(function (this: any) {
                createdXhr = this;
            ...
            wrapper.vm.startUpload([file]);

            expect(createdXhr.onload).toBeDefined();
            expect(createdXhr.onerror).toBeDefined();

            wrapper.unmount();

            // Handlers devem ter sido limpos antes do abort
            expect(createdXhr.onload).toBeNull();
            expect(createdXhr.onerror).toBeNull();
            expect(createdXhr.onabort).toBeNull();
            expect(abortSpy).toHaveBeenCalled();

            vi.restoreAllMocks();
        });

        it('impede toda emissão e mutação de estado se callbacks forem executados manualmente após unmount', async () => {
            const onUploadMock = vi.fn();
            const onErrorMock = vi.fn();
            ...
            // Chamada manual de onUploadHandler pós-desmontagem
            wrapper.vm.onUploadHandler({ xhr: { response: JSON.stringify({ file: { id: 1 } }) } });
            await wrapper.vm.$nextTick();
            ...
            // Chamada manual de onError pós-desmontagem
            wrapper.vm.onError({ xhr: { status: 500, responseText: '{"message":"Erro tardio"}' } });
            await wrapper.vm.$nextTick();

            expect(wrapper.emitted('upload-error')).toBeUndefined();
            expect(onErrorMock).not.toHaveBeenCalled();

            // Chamada manual de startUpload pós-desmontagem não faz nada
            const sendSpy = vi.spyOn(XMLHttpRequest.prototype, 'send');
            wrapper.vm.startUpload([new File([''], 'teste.pdf')]);
