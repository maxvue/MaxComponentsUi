...
describe('MaxAuthCard', () => {
    ...
    describe('Modo Customizado e Submissão Nativa', () => {
        it('renderiza o slot default quando fornecido, mantendo o header canônico', () => {
            const wrapper = mount(MaxAuthCard, {
                props: {
                ...
                slots: {
                    default: '<form class="custom-form"><input id="custom-name" /></form>'
                },
                global: {
                    stubs: {
                        MaxTitle2: {
                            template: '<div class="max-title-stub">{{ title }} - {{ subtitle }}</div>',
                            props: ['title', 'subtitle', 'icon']
                        },
                        MaxGrid: true,
                        MaxButton: true,
                        MaxInputText: true,
                        MaxInputPhoneMail: true,
                        MaxInputPhone: true,
                        MaxInputOTP: true,
            ...
            expect(wrapper.text()).toContain('Cadastro Personalizado');
            expect(wrapper.find('form.custom-form').exists()).toBe(true);
            expect(wrapper.find('#custom-name').exists()).toBe(true);
        });

        it('envolve o formulário de password em tag form com submit nativo', async () => {
            const wrapper = mountAuthCard({ email: 'user@test.com', password: 'secretpassword' });
            const form = wrapper.find('form.auth-card-form');
