...
describe('MaxUserSection', () => {
    ...
    it('exibe o nome do usuário', () => {
        ...
        expect(mountWithPinia(MaxUserSection).text()).toContain('Maria');
    });

    it('usa a empresa do usuário como subtítulo', () => {
        useUserStore().data = { id: 1, name: 'Maria', solar_company_name: 'Solar LTDA' };

        expect(mountWithPinia(MaxUserSection).text()).toContain('Solar LTDA');
    });

    it('permite sobrescrever o subtítulo', () => {
        useUserStore().data = { id: 1, name: 'Maria', solar_company_name: 'Solar LTDA' };

        const wrapper = mountWithPinia(MaxUserSection, { props: { subtitle: 'Outro' } });

        expect(wrapper.text()).toContain('Outro');
        expect(wrapper.text()).not.toContain('Solar LTDA');
    });

    it('não exibe o avatar sem usuário carregado', () => {
        expect(mountWithPinia(MaxUserSection).findComponent({ name: 'MaxUserAvatar' }).exists()).toBe(false);
    });

    it('emite logout ao acionar o item Sair', () => {
        useUserStore().data = { id: 1, name: 'Maria' };

        const wrapper = mountWithPinia(MaxUserSection);
        const items = (wrapper.vm as any).defaultItems ?? [];
        const sair = items.find?.((i: any) => i.label === 'Sair');
    ...
    it('não exibe o botão de impersonação por padrão', () => {
        useUserStore().data = { id: 1, name: 'Maria' };

        expect(mountWithPinia(MaxUserSection).find('.impersonated-btn').exists()).toBe(false);
    });
});
