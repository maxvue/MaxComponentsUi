...
export function paddingMargin(params) {
    const fullClass = String(params[0]);
    const value = getCssSize(params[1]);
    if (!fullClass.includes('p') && !fullClass.includes('m'))
        return undefined;
    ...
    if (!top && !bottom && !left && !right)
        return { [operation]: String(value) + ' !important' };
    const css = {};
    if (top)
        css[`${operation}-top`] = String(value) + ' !important';
    if (bottom)
        css[`${operation}-bottom`] = String(value) + ' !important';
    if (left)
        css[`${operation}-left`] = String(value) + ' !important';
    if (right)
        css[`${operation}-right`] = String(value) + ' !important';
    return css;
}
